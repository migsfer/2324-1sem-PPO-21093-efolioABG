...
# ***** efolioG 1b abaixo *****
'''class Mister:
    def __init__(self, Nome, Experience):
        self.Nome = Nome
        self.Experience = Experience

class Equipa(Jogador, Mister):
    def __init__(self, Nome_Equipa, Lst_Jgdrs, Mister_Nome, Mister_Exp):
        super().__init__(None, None, None, None)
        self.Nome_Equipa = Nome_Equipa
        self.Lst_Jgdrs = Lst_Jgdrs
        self.Mister_Nome = Mister_Nome
        self.Mister_Exp = Mister_Exp
...
def mostra_msg(msg):
    ...
    mymsgBOX = tk.Tk()  # create the main window
    label = tk.Label(mymsgBOX, text=msg, font="Arial 20")
    label.pack()  # display the label on the window
    # get screen width and height and center
    screen_width = mymsgBOX.winfo_screenwidth()
    screen_height = mymsgBOX.winfo_screenheight()
    x = (screen_width - mymsgBOX.winfo_reqwidth()) // 2
    y = (screen_height - mymsgBOX.winfo_reqheight()) // 2

    mymsgBOX.geometry("{}x100+{}+{}".format(20*len(msg), x, y))
    mymsgBOX.bind("<Key>", close_window)  # Bind the close_window function to any key press
    mymsgBOX.mainloop()  # run the main event loop to display the window
